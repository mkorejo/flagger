...
func (c *Controller) advanceCanary(name string, namespace string, skipLivenessChecks bool) {
	...
	if cd.Status.Phase == flaggerv1alpha3.CanaryPhaseProgressing &&
		...
		if err := canaryController.SyncStatus(cd, flaggerv1alpha3.CanaryStatus{Phase: flaggerv1alpha3.CanaryPhaseFailed, CanaryWeight: 0}); err != nil {
			c.logger.With("canary", fmt.Sprintf("%s.%s", cd.Name, cd.Namespace)).Errorf("%v", err)
			return
		}

		c.recorder.SetStatus(cd, flaggerv1alpha3.CanaryPhaseFailed)
		c.runPostRolloutHooks(cd, flaggerv1alpha3.CanaryPhaseFailed)
		return
	}
